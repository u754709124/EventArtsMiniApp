...
人员后台的下方标签和描述均为可选：空标签规范化为 `[]`，空白描述规范化为 `""`，前台不创建对应节点。人员列表保持两列固定宽度，卡片按实际内容自适应高度并分配到当前较短列，卡片正文底部保留稳定留白。

## 独立详情页管理

详情页是独立、可复用、可统一管理的内容实体。后台侧栏提供“详情页管理”，业务表单只选择 `detailPageId`，不再内嵌完整详情配置。公告、首页 BANNER、人员、案例、近日活动、文章和“详情页直达”菜单可以共享同一详情页；被任一业务记录引用的详情页不能删除。

后台信息架构包含“数据看板 / 首页运营 / 内容管理 / 素材管理 / 账号安全 / 定时任务 / 系统配置”，菜单、面包屑和路由高亮来自同一导航配置。人员、案例和近日活动使用独立新增/编辑页，短首页运营表单保留抽屉；详见 `docs/design/admin-navigation-and-forms.md`。

后台“定时任务”展示服务器固定目录中的管理员会话清理、管理员消息清理、访问统计清理和 EdgeOne 预热对账，包含说明、五段 cron、计划下次执行和最近真实执行状态。真实 API 进程启动时会按 `Asia/Shanghai` 启动内置调度器，关闭时清理计时器；每次自动执行和“立即执行”都复用统一 runner 并写入状态表。“立即执行”需要确认且只提交任务 key，数据库租约会阻止同一任务并发。四条清理/对账 CLI 仅保留用于人工恢复。
...
近日活动是独立于 `activity_case` 的首页内容实体，不迁移旧案例，也不重命名 `/cases`、`activity_case` 或后台“案例管理”。后台“内容管理 / 近日活动管理”提供完整 CRUD、搜索、分页、启停、拖拽排序和删除确认；字段包括标题、标签、封面、摘要、活动日期、地点、可空详情页、排序和状态。

首页 `GET /api/client/home` 返回 `recentActivities`，只包含启用记录，按 `sortOrder ASC, id ASC` 展示全部，不按活动日期过滤；小程序首页将“近日活动”放在“活动方案”上方，复用紧凑案例卡片视觉，未绑定 `detailPageId` 的卡片不可点击。媒体删除保护包含 `recentActivity.cover` / `recent_activity_cover`，详情页引用保护包含 sourceType `recent_activity`。seed 会创建三条近日活动示例，但仍不创建或覆盖管理员。
...
- `SUPER_ADMIN` 账户管理：可管理其他 `SUPER_ADMIN`、`ADMIN` 和 `USER`，但不能通过 Web 自助管理自身，也不能通过一次性链接新建 `SUPER_ADMIN`；系统始终保留至少一个启用的 `SUPER_ADMIN`。
...
- `USER` 忘记密码：由 `ADMIN` 或 `SUPER_ADMIN` 生成一次性恢复链接；`ADMIN` 只能管理 `USER`，且只能下放自己已有的非敏感业务菜单权限。
...
管理端会在上传前读取图片/视频尺寸并分片计算 MD5；服务端会重新计算并校验。MD5 已存在时直接复用资源；Banner、菜单图标、案例封面、近日活动封面、文章封面等固定槽位仅展示推荐尺寸，上传和最终保存不限制图片尺寸，但仍会校验资源类型和可解析元数据。正在被站点配置、Banner、菜单、案例封面、近日活动封面、文章封面、公共详情 BANNER/富文本或人员头像引用的资源不可删除。
...
- 后台“账号安全 / 备份与恢复”支持创建 v3 非身份数据备份、查看类型和数据范围、逐条下载受保护的 `.tar.gz`、删除、导入 `.tar`/`.tar.gz` 外部备份并执行 `RESTORE_FULL_BACKUP` 二次确认恢复。新备份不包含后台账户、角色、菜单权限、会话、重置链接、个人通知、客户端会话、访问事件、任务状态和操作日志；旧 v1/v2 归档仍可兼容预检和恢复，但候选身份永远不会成为线上身份。下载使用管理员 Bearer 鉴权且不暴露 `BACKUP_DIR` 公共 URL；恢复成功会撤销所有管理员会话和未使用重置链接。
- 生产调度器运行管理员会话清理、管理员消息清理、访问统计清理和 EdgeOne 预热对账；四条清理/对账 CLI 与后台立即执行复用同一 runner 并记录最近状态。每日自动备份任务已移除，新备份只通过后台手动创建或恢复流程的安全快照产生；已有 `backupKind: "automatic"` 归档仍可查看、下载、删除和恢复。
