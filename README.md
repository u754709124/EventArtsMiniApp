...
后台“定时任务”展示服务器固定目录中的管理员会话清理、管理员消息清理、访问统计清理、EdgeOne 预热对账和自动备份，包含说明、五段 cron、计划下次执行和最近真实执行状态。真实 API 进程启动时会按 `Asia/Shanghai` 启动内置调度器，关闭时清理计时器；每次自动执行和“立即执行”都复用统一 runner 并写入状态表。“立即执行”需要确认且只提交任务 key，数据库租约会阻止同一任务并发。四条清理/对账 CLI 仅保留用于人工恢复；自动备份由内置调度器和后台立即执行入口触发，不应再配置重复的外部 cron。
...
- 生产调度器继续调用 `admin:sessions:cleanup`、`admin:notifications:cleanup`、`analytics:cleanup`、`edgeone:prefetch:reconcile` 和 `automatic-backup`；其中自动备份每天 `0 0 * * *` 按北京时间创建 v3 非身份备份并只保留最新 3 个自动备份，手动、导入和恢复前安全快照不参与自动清理。四条清理/对账 CLI 与后台立即执行复用同一 runner 并记录最近状态；自动备份不需要额外外部 cron，真实 API 进程已内置调度器。
