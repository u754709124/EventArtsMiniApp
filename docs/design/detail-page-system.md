...
`DetailPageRenderer` 以共享 `rendererKey` 查找 exhaustively typed registry。`BannerRichTextRenderer` 将 BANNER 顶部放在返回按钮上边缘上方约 `4–8px`（按状态栏与跨端按钮实际尺寸动态换算），导航绝对覆盖在图片上方；单图不 circular。导航使用暖白到透明的多段渐变，返回按钮仍与单富文本详情页保持相同安全位置。Hero 文字组从导航底部再向下保留 `8px`，避免与返回按钮冲突并维持在图片上半区；骨架和后台移动预览复用同一层级关系。`RichTextRenderer` 只渲染正常导航和内容，完全没有 BANNER、Hero、页码、BANNER skeleton、高度或负 margin DOM。
...
- BANNER 以 `424rpx` 为最小高度而非固定最终高度；其上方安全导航独立占据正常流，BANNER 顶边不进入状态栏或胶囊区域。Hero 参与 BANNER 内部流式布局，相对图片顶部使用固定 `48px`（750rpx 设计宽度下约 `96rpx`）偏移，落在图片上半部分并避开 `72rpx` 羽化区；设备状态栏或胶囊高度只移动 BANNER 整体，不再改变图片内部构图。类型/标题、宣传语、主标签、多标签、地点和元数据组成同一个左对齐纵向文字组，共用该起点并保持既有组内间距；右下计数器独立锚定，文字组为其预留横向安全宽度。底部显式预留 `42rpx` 首卡重叠量加 `24rpx` 可见间距。Swiper、遮罩和计数器绝对铺在最终 BANNER 高度内。
- Admin 的 375px 移动端预览镜像该区块顺序：独立 `20px + 44px` 安全导航、`212px` BANNER 最小高度、Hero 相对图片顶部 `48px`、首卡负重叠 `21px`；正文为 `15px`，富文本标题为 `17px/1.35/700`。
