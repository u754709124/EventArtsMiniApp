...
- 已实现 Prisma schema，覆盖统一资源标签、案例有序媒体关联与稳定 seed 身份记录，以及 `admin_users`、`site_config`、`media_assets`、`announcements`、`banners`、`menu_items`、`artists`、`activity_cases`、`page_view_events`、`operation_logs`。
- 已实现 SQLite bootstrap helper、内容寻址 seed 资源和稳定业务 seed 身份，默认管理员 `admin/admin123456`、默认站点配置、公告、五类菜单、三条精选案例和三类人员可安全重复初始化。
- 已实现用途无关的统一媒体库：流式临时文件、服务端 MD5、Sharp 图片探测、ffprobe MP4 探测、随机存储名、标签和全局唯一资源名。
- 已实现共享 `MediaFieldKey` 规则，所有业务保存接口重新验证资源存在性、类型和尺寸；案例详情改用 `ActivityCaseMedia` 有序关联。
- 已实现资源查询/详情/标签/MD5 查询/名称检查/元数据编辑/未使用扫描/批量删除接口，并在删除前重新统计全部引用。
