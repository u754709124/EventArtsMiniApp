  ...
  return (
    <View className="page">
      ...
      {items.length === 0 ? (
        ...
        items.map((item) => (
          <View key={item.id} className="case-card card" onClick={() => Taro.navigateTo({ url: `/pages/cases/detail?id=${item.id}` })}>
            <AppImage className="case-card__image" src={item.coverUrl} fallback={generatedAssets.placeholderCase} />
