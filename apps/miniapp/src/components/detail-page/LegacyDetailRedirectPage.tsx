...
import { PullDownRefreshIndicator } from "../PageState";
...
export function LegacyDetailRedirectPage<T extends LegacyDetailRecord>({
  ...
  const refreshing = usePullDownRefreshState(reload);

  if (state.status === "loading") {
    return (
      <>
        {refreshing && <PullDownRefreshIndicator />}
        <DetailPageSkeleton layout="richText" fallbackTabUrl={fallbackTabUrl} title={loadingTitle} />
      </>
    );
  }
