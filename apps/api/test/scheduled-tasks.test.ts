...
describe("scheduled task admin API", () => {
  it("requires auth, returns no-store catalog data, runs by task key, and rejects arbitrary input", async () => {
    ...
    const backupEntriesBefore = await readdir(backupDir);
    const removedTask = await app!.inject({
      method: "POST",
      url: "/api/admin/scheduled-tasks/automatic-backup/run",
      headers: auth(token),
      payload: {}
    });
    expect(removedTask.statusCode).toBe(404);
    expect(removedTask.json()).toMatchObject({
      success: false,
      error: { code: "SCHEDULED_TASK_NOT_FOUND" }
    });
    expect(await readdir(backupDir)).toEqual(backupEntriesBefore);
